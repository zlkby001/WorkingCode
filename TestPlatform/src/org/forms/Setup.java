			...
			public void widgetSelected(SelectionEvent e) {
			 if(Password.count == 0){	
				...
				if (LibMeasure.whitemode == 1) {
					...
					switch (ret) {
					case 0:
						//audit.log_record(MainForm.UserIdentity, 4, "关闭进程管控-设置");
						audit.log_record(MainForm.UserIdentity,UserInfo.userAcc, "进程管控", "关闭进程管控-设置");
					...
					//	audit.log_record(MainForm.UserIdentity, 4, "开启进程管控-设置");
						audit.log_record(MainForm.UserIdentity,UserInfo.userAcc, "进程管控", "开启进程管控-设置");
		...
		btn_selfstart.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
			  if(Password.count == 0){	
				...
				//	audit.log_record(MainForm.UserIdentity, 6, "关闭监控自启动-设置");
					audit.log_record(MainForm.UserIdentity,UserInfo.userAcc, "监控自启动", "关闭监控自启动-设置");}
				else{
					try {
						FileWriter wr=new FileWriter(selfstartfile);
						wr.write("1");
				...
				//	audit.log_record(MainForm.UserIdentity, 6, "开启监控自启动-设置");
					audit.log_record(MainForm.UserIdentity,UserInfo.userAcc, "监控自启动", "开启监控自启动-设置");
