		...
		btn_CloseMonitor.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
		     if(Password.count == 0){		
				...
				//	audit.log_record(MainForm.UserIdentity, 5, "关闭USB管控-设置");
					audit.log_record(MainForm.UserIdentity,UserInfo.userAcc, "USB管控", "关闭USB管控-设置");
				}
				else{
					try {
						FileWriter wr=new FileWriter(usbcontrolfile);
					...
					//audit.log_record(MainForm.UserIdentity, 5, "开启USB管控-设置");
					audit.log_record(MainForm.UserIdentity,UserInfo.userAcc, "USB管控", "开启USB管控-设置");
				}
				mb.open();
