...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	String newuserid=request.getParameter("oid");
	String url="servlet/restoreServlet?newuserid="+newuserid;
	String restoreurl="servlet/dorestoreServlet?newuserid="+newuserid+"&olduserid=";
%>
...
<script type="text/javascript">
    ...
    function renderOperate(value, p, record){
    	var html = "<a href='javascript:void(0)' onclick='restore()'>[修复]</a>" ;
        return html;
    }
    ...
    function restore(){
		...
		var r = row[0];  
		var val = confirm("您确定要执行此修复操作吗??");
		if (val) {
		    var result = ajaxSyncCall('<%=restoreurl%>' + r.data.id, null);
		    if (result.success) {
		    	alert("已成功修复终端!");
