...
public class addAllWhitelistServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public addAllWhitelistServlet() {
		super();
	}
...
//				fb.setSoftwareName("锟斤拷锟斤拷锟�);
		...
		if(result){
			if(loginservlet.usertype.equalsIgnoreCase("manager")){
			  ...
			  audit.log_record(userid, loginservlet.usertype, loginservlet.name1, "导入白名单", "远程日志选中导入完成");
			request.setAttribute("result","添加白名单成功！");
		}else 
			
			
			request.setAttribute("result", "添加白名单失败！");
		if(res == true)
